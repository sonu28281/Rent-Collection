  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-4 py-2.5 rounded-t-2xl sticky top-0 shadow-sm">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-bold flex items-center gap-1.5">📝 {t('Submit Payment', 'पेमेंट सबमिट')}</h3>
            <button onClick={onClose} className="text-white hover:bg-white/20 rounded-full w-7 h-7 flex items-center justify-center transition">
              ✕
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-2.5">
          {error && (
            <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg p-2">
              <p className="text-red-700 dark:text-red-300 text-xs font-medium">⚠️ {error}</p>
            </div>
          )}

          <div>
            <p className="text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">{t('Payable Summary', 'देय भुगतान सारांश')}</p>
            <div className="grid grid-cols-3 gap-1.5">
              <div className="bg-blue-50 dark:bg-blue-950/30 rounded-lg p-1.5 ring-1 ring-blue-200 dark:ring-blue-800 text-center">
                <p className="text-[9px] font-semibold text-blue-600 dark:text-blue-400 uppercase">{t('Rent', 'किराया')}</p>
                <p className="text-xs font-bold text-blue-700 dark:text-blue-300">₹{(isMultiRoom ? getBreakdownTotals().rentAmount : (parseFloat(formData.rentAmount) || 0)).toLocaleString('en-IN')}</p>
              </div>
              <div className="bg-purple-50 dark:bg-purple-950/30 rounded-lg p-1.5 ring-1 ring-purple-200 dark:ring-purple-800 text-center">
                <p className="text-[9px] font-semibold text-purple-600 dark:text-purple-400 uppercase">{t('Electricity', 'बिजली')}</p>
                <p className="text-xs font-bold text-purple-700 dark:text-purple-300">₹{(isMultiRoom ? getBreakdownTotals().electricityAmount : Number(calculateElectricity())).toFixed(2)}</p>
              </div>
              <div className="bg-green-50 dark:bg-green-950/30 rounded-lg p-1.5 ring-1 ring-green-200 dark:ring-green-800 text-center">
                <p className="text-[9px] font-semibold text-green-600 dark:text-green-400 uppercase">{t('Total', 'कुल')}</p>
                <p className="text-xs font-bold text-green-700 dark:text-green-300">₹{expectedTotal.toFixed(2)}</p>
              </div>
            </div>
          </div>
            ...
            {isMultiRoom ? (
              <div className="space-y-2">
                {formData.roomBreakdown.map((entry, index) => {
                  ...
                  return (
                    <div key={entry.roomNumber} className="border border-gray-200 dark:border-slate-700 rounded-lg p-2 bg-gray-50 dark:bg-slate-700/50">
                      <span className="inline-flex items-center justify-center min-w-[2rem] px-1.5 py-0.5 rounded-md text-[11px] font-bold bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-100 shadow-sm ring-1 ring-gray-200 dark:ring-slate-600 mb-1.5">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          ...
                          min="0"
                          className="w-full px-3 py-1.5 text-sm border-2 border-gray-300 dark:border-slate-600 rounded-lg bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 cursor-not-allowed"
                        ...
                        <input
                          ...
                          className="w-full px-3 py-1.5 text-sm border-2 border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-green-500"
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-1.5 border-2 border-green-300 dark:border-green-700 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-green-500 font-bold text-base"
              min="0"
              step="0.01"
            ...
            {paidAmountValue > 0 && (
              <div className={`mt-1.5 rounded-lg border p-2 text-xs ${isPartialPayment ? 'bg-amber-50 dark:bg-amber-950/30 border-amber-300 dark:border-amber-800 text-amber-900 dark:text-amber-300' : 'bg-green-50 dark:bg-green-950/30 border-green-300 dark:border-green-800 text-green-900 dark:text-green-300'}`}>
                {isPartialPayment ? (
                  <>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-1.5 text-sm border-2 border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-green-500 font-mono"
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-2 py-1.5 text-sm border-2 border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-green-500 file:mr-2 file:py-1 file:px-2.5 file:rounded-md file:border-0 file:bg-green-100 dark:file:bg-green-900/50 file:text-green-700 dark:file:text-green-300 file:font-semibold file:text-xs"
              required
            />
            {formData.screenshot && (
              <div className="mt-2">
                <img
                  ...
                  className="max-h-36 w-auto rounded-lg border border-gray-300 dark:border-slate-600 shadow-sm"
                />
              </div>
            )}
          ...
          <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-2.5">
            <p className="text-xs font-semibold text-blue-900 dark:text-blue-300 mb-1">ℹ️ {t('What happens next?', 'आगे क्या होगा?')}</p>
            <ul className="text-[11px] text-blue-800 dark:text-blue-400 space-y-0.5">
          ...
          <div className="flex gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2 text-sm border-2 border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-200 font-semibold rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 transition"
            >
              {t('Cancel', 'रद्द करें')}
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="flex-1 px-4 py-2 text-sm bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold rounded-xl shadow-lg transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
