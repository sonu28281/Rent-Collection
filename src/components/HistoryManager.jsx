...
const HistoryManager = () => {
  ...
  const [expandedHistoryRooms, setExpandedHistoryRooms] = useState(new Set()); // room-wise grouping toggle
  const toggleHistoryRoom = (roomKey) => {
    setExpandedHistoryRooms((prev) => {
  ...
  return (
    <div className="p-4 lg:p-6 pb-24">
      {/* Header */}
      <div className="mb-4">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 mb-1">📚 Payment History Manager</h2>
        <p className="text-sm text-gray-600">View, edit, and manage historical payment records</p>
      </div>

      <div className="space-y-2 mb-4">
        <div className="card bg-white/95 dark:bg-slate-800/95 backdrop-blur border border-indigo-200 dark:border-indigo-800 shadow-sm">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 mb-2">
            <div>
              <h3 className="text-sm md:text-base font-bold text-gray-800 dark:text-slate-100">💰 Overall Summary - {selectedYear}</h3>
              <p className="text-[11px] md:text-xs text-gray-600 dark:text-slate-400">
                Floor 1: {floor1Count} • Floor 2: {floor2Count} • Total: {payments.length}
              </p>
            </div>
            <div className="text-[11px] md:text-xs font-semibold px-2.5 py-1 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-900 dark:text-indigo-300 w-fit">
              Full Year Collection
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
            <div className="bg-blue-50 dark:bg-blue-950/30 rounded-md px-2.5 py-2 border border-blue-200 dark:border-blue-800">
              <p className="text-[10px] text-blue-700/80 dark:text-blue-400/80 font-semibold">Rent</p>
              <p className="text-sm md:text-base font-bold text-blue-700 dark:text-blue-400">₹{overallTotals.rent.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-purple-50 dark:bg-purple-950/30 rounded-md px-2.5 py-2 border border-purple-200 dark:border-purple-800">
              <p className="text-[10px] text-purple-700/80 dark:text-purple-400/80 font-semibold">Electricity</p>
              <p className="text-sm md:text-base font-bold text-purple-700 dark:text-purple-400">₹{overallTotals.electricity.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-green-50 dark:bg-green-950/30 rounded-md px-2.5 py-2 border border-green-200 dark:border-green-800">
              <p className="text-[10px] text-green-700/80 dark:text-green-400/80 font-semibold">Grand Total</p>
              <p className="text-sm md:text-base font-bold text-green-700 dark:text-green-400">₹{overallTotals.total.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-teal-50 dark:bg-teal-950/30 rounded-md px-2.5 py-2 border border-teal-200 dark:border-teal-800">
              <p className="text-[10px] text-teal-700/80 dark:text-teal-400/80 font-semibold">Paid</p>
              <p className="text-sm md:text-base font-bold text-teal-700 dark:text-teal-400">₹{overallTotals.paidAmount.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-orange-50 dark:bg-orange-950/30 rounded-md px-2.5 py-2 border border-orange-200 dark:border-orange-800">
              <p className="text-[10px] text-orange-700/80 dark:text-orange-400/80 font-semibold">Balance</p>
              <p className={`text-sm md:text-base font-bold ${overallTotals.balance < 0 ? 'text-red-700 dark:text-red-400' : 'text-orange-700 dark:text-orange-400'}`}>
        ...
        <div className="card bg-white/95 dark:bg-slate-800/95 backdrop-blur border border-gray-200 dark:border-slate-700 shadow-sm">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-2 mb-3">
            <div className="lg:col-span-3">
            <label className="block text-xs font-semibold text-gray-600 dark:text-slate-400 mb-1">Year</label>
            <select
              value={selectedYear}
              onChange={(e) => setSelectedYear(Number(e.target.value))}
              className="w-full px-2.5 py-1.5 text-sm border border-gray-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-primary focus:border-transparent"
            >
              {years.map(year => (
            ...
            <div className="lg:col-span-3">
              <label className="block text-xs font-semibold text-gray-600 dark:text-slate-400 mb-1">Floor</label>
              <select
                value={selectedFloor}
            ...
            <label className="block text-xs font-semibold text-gray-600 dark:text-slate-400 mb-1">Tenant Filter</label>
            <select
              value={tenantFilter}
          ...
          {!historyEditDeleteEnabled && (
            <div className="mt-3 bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-200 dark:border-yellow-800 rounded-lg px-3 py-2">
        ...
        <>
          ...
          {(() => {
            ...
            const renderFloorSection = (floorNum, floorPayments, colorScheme) => {
              ...
              // Group this floor's records by room so history is browsed room-by-room
              // instead of one long flat list of every payment record.
              const roomNumbersForFloor = [...new Set(floorPayments.map((p) => p.roomNumber))]
                .sort((a, b) => Number(a) - Number(b));
              const getRoomPayments = (room) =>
                floorPayments
                  .filter((p) => p.roomNumber === room)
                  .sort((a, b) => (Number(b.year) - Number(a.year)) || (Number(b.month) - Number(a.month)));
              const getRoomSummary = (room) => {
                const rp = getRoomPayments(room);
                const latest = rp[0];
                const totalCollected = rp.reduce((sum, p) => sum + (Number(p.paidAmount) || 0), 0);
                const totalDue = rp.reduce((sum, p) => {
                  const rent = Number(p.rent) || 0;
                  const units = Number(p.units) || ((Number(p.currentReading) || 0) - (Number(p.oldReading) || 0));
                  const electricity = Number(p.electricity) || (units * (Number(p.ratePerUnit) || 0));
                  const total = Number(p.total) || Number(p.totalAmount) || (rent + electricity);
                  return sum + Math.max(total - (Number(p.paidAmount) || 0), 0);
                }, 0);
                const latestTenant = latest?.roomStatus === 'vacant' ? '' : (latest?.tenantNameSnapshot || latest?.tenantName || 'Unknown');
                const distinctTenants = new Set(
                  rp.filter((p) => p.roomStatus !== 'vacant').map((p) => (p.tenantNameSnapshot || p.tenantName || '').trim()).filter(Boolean)
                );
                return { records: rp.length, totalCollected, totalDue, latestTenant, multiTenant: distinctTenants.size > 1 };
              };

              return (
                <div key={`floor-${floorNum}`} className="mb-5">
                  {/* Floor Summary Card */}
                  <div className={`card mb-3 bg-gradient-to-r ${colorScheme.gradient} ${colorScheme.darkGradient} border ${colorScheme.border} ${colorScheme.darkBorder}`}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className={`text-xs font-semibold px-3 py-1.5 rounded-full ${colorScheme.badge} ${colorScheme.darkBadge}`}>
                        {floorPayments.length} record{floorPayments.length !== 1 ? 's' : ''}
                      </div>
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-5 gap-2.5">
                      <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border border-blue-200 dark:border-blue-800">
                        <p className="text-[11px] text-gray-600 dark:text-slate-400 mb-0.5">Total Rent</p>
                        <p className="text-base md:text-lg font-bold text-blue-600 dark:text-blue-400">₹{totals.rent.toLocaleString('en-IN')}</p>
                      </div>
                      <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border border-purple-200 dark:border-purple-800">
                        <p className="text-[11px] text-gray-600 dark:text-slate-400 mb-0.5">Total Electricity</p>
                        <p className="text-base md:text-lg font-bold text-purple-600 dark:text-purple-400">₹{totals.electricity.toLocaleString('en-IN')}</p>
                      </div>
                      <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border border-green-200 dark:border-green-800">
                        <p className="text-[11px] text-gray-600 dark:text-slate-400 mb-0.5">Grand Total</p>
                        <p className="text-base md:text-lg font-bold text-green-600 dark:text-green-400">₹{totals.total.toLocaleString('en-IN')}</p>
                      </div>
                      <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border border-teal-200 dark:border-teal-800">
                        <p className="text-[11px] text-gray-600 dark:text-slate-400 mb-0.5">Total Paid</p>
                        <p className="text-base md:text-lg font-bold text-teal-600 dark:text-teal-400">₹{totals.paidAmount.toLocaleString('en-IN')}</p>
                      </div>
                      <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border border-orange-200 dark:border-orange-800">
                        <p className="text-[11px] text-gray-600 dark:text-slate-400 mb-0.5">Total Balance</p>
                        <p className={`text-base md:text-lg font-bold ${totals.balance < 0 ? 'text-red-600 dark:text-red-400' : 'text-orange-600 dark:text-orange-400'}`}>
                  ...
                  {/* Floor Table / Cards — grouped room-by-room */}
                  {isMobileViewport ? (
                    <div className="space-y-3">
                      {roomNumbersForFloor.map((room) => {
                        const roomKey = `${floorNum}-${room}`;
                        const isRoomExpanded = expandedHistoryRooms.has(roomKey);
                        const summary = getRoomSummary(room);
                        return (
                          <div key={room} className="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm overflow-hidden">
                            <button
                              type="button"
                              onClick={() => toggleHistoryRoom(roomKey)}
                              className="w-full flex items-center justify-between gap-2 p-3 text-left hover:bg-gray-50 dark:hover:bg-slate-700/60 transition"
                            >
                              <div className="flex items-center gap-2 min-w-0">
                                <span className="inline-flex items-center justify-center min-w-[2.5rem] px-2 py-1 rounded-lg text-sm font-bold bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 ring-1 ring-indigo-200 dark:ring-indigo-800">
                                  {room}
                                </span>
                                <div className="min-w-0">
                                  <p className="text-sm font-bold text-gray-900 dark:text-slate-100 truncate">
                                    {summary.latestTenant || 'Vacant'}
                                    {summary.multiTenant && <span className="ml-1 text-[10px] font-normal text-indigo-600 dark:text-indigo-400">· multiple tenants</span>}
                                  </p>
                                  <p className="text-[11px] text-gray-500 dark:text-slate-400">
                                    {summary.records} record{summary.records !== 1 ? 's' : ''} · Collected ₹{summary.totalCollected.toLocaleString('en-IN')}
                                    {summary.totalDue > 0 ? ` · Due ₹${summary.totalDue.toLocaleString('en-IN')}` : ''}
                                  </p>
                                </div>
                              </div>
                              <span className={`text-gray-400 dark:text-slate-500 transition-transform ${isRoomExpanded ? 'rotate-180' : ''}`}>▼</span>
                            </button>

                            {isRoomExpanded && (
                              <div className="border-t border-gray-100 dark:border-slate-700 p-3 space-y-3 bg-gray-50/60 dark:bg-slate-900/30">
                                {getRoomPayments(room).map((payment) => {
                        ...
                        return (
                          <div key={payment.id} className="rounded-lg border border-gray-200 bg-white p-3">
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0">
                                <p className="text-base font-bold text-gray-900 leading-tight">{monthYearLabel}</p>
                                {!isEditing && (
                    ...
                    <table className="w-full text-xs md:text-sm">
                      <thead className="bg-gradient-to-r from-gray-50 to-slate-100 dark:from-slate-800 dark:to-slate-800 sticky top-0">
                        <tr>
                          ...
                          <th className="px-2.5 py-2.5 text-left text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Room</th>
                          <th className="px-2.5 py-2.5 text-left text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Status</th>
                          <th className="px-2.5 py-2.5 text-left text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">
                            Tenant {selectedMonth !== 'all' ? `(${MONTHS.find(m => m.num === selectedMonth)?.name})` : '(All Months)'}
                          </th>
                          <th className="px-2.5 py-2.5 text-right text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Rent</th>
                          <th className="px-2.5 py-2.5 text-right text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Old Reading</th>
                          <th className="px-2.5 py-2.5 text-right text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Current Reading</th>
                          <th className="px-2.5 py-2.5 text-right text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Units</th>
                          <th className="px-2.5 py-2.5 text-right text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Rate</th>
                          <th className="px-2.5 py-2.5 text-right text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Electricity</th>
                          <th className="px-2.5 py-2.5 text-right text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Total</th>
                          <th className="px-2.5 py-2.5 text-right text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Paid</th>
                          <th className="px-2.5 py-2.5 text-center text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Status</th>
                          <th className="px-2.5 py-2.5 text-center text-[10px] md:text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-slate-400">Actions</th>
                        </tr>
                      </thead>
                        ...
                        {roomNumbersForFloor.map((room) => {
                          ...
                          return (
                            <Fragment key={room}>
                              <tr
                                onClick={() => toggleHistoryRoom(roomKey)}
                                className="cursor-pointer bg-indigo-50/70 dark:bg-indigo-950/25 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 transition-colors"
                              >
                                <td className="px-2 py-2.5 text-center" onClick={(e) => e.stopPropagation()}>
                                  <input
                                    type="checkbox"
                                    checked={getRoomPayments(room).every((p) => selectedIds.has(p.id))}
                                    onChange={(e) => {
                                      const roomIds = getRoomPayments(room).map((p) => p.id);
                                      if (e.target.checked) {
                                        setSelectedIds(new Set([...selectedIds, ...roomIds]));
                                      } else {
                                        const newSelected = new Set(selectedIds);
                                ...
                                <td className="px-2.5 py-2.5" colSpan={13}>
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <span className={`text-indigo-500 dark:text-indigo-400 transition-transform ${isRoomExpanded ? 'rotate-90' : ''}`}>▶</span>
                                    ...
                                    <span className="font-bold text-sm text-gray-900 dark:text-slate-100">{summary.latestTenant || 'Vacant'}</span>
                                    {summary.multiTenant && <span className="text-[10px] text-indigo-600 dark:text-indigo-400">· multiple tenants</span>}
                              ...
                              {isRoomExpanded && getRoomPayments(room).map((payment) => {
                  const isEditing = editingId === payment.id;
                  ...
                  return (
                    <tr key={payment.id} className={`transition-colors ${isAutoAssigned ? 'bg-blue-50 dark:bg-blue-950/30 hover:bg-blue-100 dark:hover:bg-blue-900/40' : 'hover:bg-gray-50 dark:hover:bg-slate-700/60'}`}>
            ...
            const floor1Colors = {
              ...
              darkGradient: 'dark:from-green-950/40 dark:to-emerald-950/30',
              border: 'border-green-200',
              darkBorder: 'dark:border-green-800',
              badge: 'bg-green-200 text-green-800',
              darkBadge: 'dark:bg-green-900/50 dark:text-green-300'
            };

            const floor2Colors = {
              gradient: 'from-purple-50 to-indigo-50',
              darkGradient: 'dark:from-purple-950/40 dark:to-indigo-950/30',
              border: 'border-purple-200',
              darkBorder: 'dark:border-purple-800',
              badge: 'bg-purple-200 text-purple-800',
              darkBadge: 'dark:bg-purple-900/50 dark:text-purple-300'
            };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b dark:border-slate-700">
              <h3 className="text-xl font-bold text-gray-800 dark:text-slate-100">Confirm CSV Import</h3>
              <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">Review the data before importing</p>
            </div>
            ...
            <div className="p-6 overflow-auto flex-1">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs border dark:border-slate-600">
                  <thead className="bg-gray-100 dark:bg-slate-700">
                    <tr>
                      {Object.keys(importPreview[0] || {}).map(key => (
                        <th key={key} className="px-2 py-1 text-left border dark:border-slate-600 text-gray-700 dark:text-slate-200">{key}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {importPreview.slice(0, 10).map((row, idx) => (
                      <tr key={idx} className="border-b dark:border-slate-700">
                        {Object.values(row).map((val, vidx) => (
                          <td key={vidx} className="px-2 py-1 border dark:border-slate-600 text-gray-800 dark:text-slate-200">{val}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-slate-400">This action is permanent and data cannot be recovered</p>
              </div>
            </div>
            ...
            <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4">
              ...
              <ul className="text-red-700 dark:text-red-400 space-y-1 ml-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2 bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-200 rounded-lg hover:bg-gray-300 dark:hover:bg-slate-600 font-medium"
              >
                Cancel
              </button>
              <button
